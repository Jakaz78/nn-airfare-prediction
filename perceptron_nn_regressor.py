...
class MultiLayerPerceptron:
    ...
    def fit2(self, X, y, X_val=None, y_val=None, num_epochs=1000, verbose=False, 
         early_stopping=True, patience=50, min_delta=0.0001, convergence_threshold=1e-6):
        """
        Trening sieci neuronowej z pełną historią wyników dla każdej epoki.

        Returns:
        --------
        dict: Historia treningu z metrykami końcowymi
        list: Lista słowników z wynikami cząstkowymi dla każdej epoki
        """
        X = X.astype(np.float64)
        ...
        for epoch in range(num_epochs):
            ...
            if np.isnan(train_loss):
                print(f"Warning: NaN loss detected at epoch {epoch + 1}. Training stopped.")
                if best_weights:
                    self.weights = best_weights
                self.history = history
            ...
            if train_loss < convergence_threshold:
                print(f"Convergence achieved at epoch {epoch + 1}. Training MSE: {train_loss:.8f}")
                if early_stopping:
                    _, val_predictions = self.forward_propagation(X_val)
                    val_loss = np.mean((val_predictions - y_val) ** 2)
                    epoch_result['val_mse'] = val_loss
                results.append(epoch_result)
                self.history = epoch_result
                return epoch_result, results
            ...
            if early_stopping:
                ...
                if counter >= patience:
                    print(f"Early stopping na epoce {epoch + 1}/{num_epochs}. {patience} epok bez poprawy. Najlepszy val_MSE: {best_loss:.4f}")
                    self.weights = best_weights
                    self.history = epoch_result
                    results.append(epoch_result)
                    return epoch_result, results
        ...
        self.history = results[-1] if results else {}
        return results
